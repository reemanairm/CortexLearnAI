...
import { Mail, Lock, ArrowRight, BrainCircuit, Sparkles } from "lucide-react";
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // use AuthContext.login so the global auth state updates immediately
      ...
      if (res && (res.token || res.data?.token)) {
        ...
        // fallback: if context login didn't return token but succeeded, still navigate
        toast.success("Welcome back 🚀");
        navigate("/dashboard");
      }
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="bg-slate-900/60 backdrop-blur-xl p-8 sm:p-10 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-white/10 relative overflow-hidden group">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="button"
              onClick={() => window.location.href = "https://cortexlearnai.onrender.com/api/auth/google"}
          ...
          <div className="mt-8 flex items-center justify-center gap-2 text-sm text-slate-400 font-medium">
            <span>New to CortexLearn AI?</span>
            <Link autoFocus={false} to="/register" className="text-indigo-400 hover:text-indigo-300 hover:underline transition-all">Create an account</Link>
          </div>
        ...
        <p className="text-xs text-slate-600 font-medium text-center mt-6">
          By signing in, you agree to our <a href="#" className="hover:text-slate-400 transition-colors">Terms of Service</a> and <a href="#" className="hover:text-slate-400 transition-colors">Privacy Policy</a>.
        </p>
